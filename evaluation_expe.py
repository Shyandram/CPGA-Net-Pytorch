...
def get_data(ori_root, lowlight_root, ):
    ll = []
    ori = []
    image_name_list = [x for x in os.listdir(lowlight_root) if is_image_file(x)]
    FileE = '.JPG'
    for key in image_name_list:
        key = key.split("/")[-1]
        if os.name == 'nt':
            key = key.split("\\")[-1]
        ori.append(os.path.join(ori_root, '_'.join(key.split('_')[:-1])+FileE))
        ll.append(os.path.join(lowlight_root,key))
    return ll, ori
...
def measure_dirs(dirA, dirB, use_gpu, verbose=False):
    ...
    paths_A, paths_B = get_data(dirA, dirB)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-dirA', default='/mnt/c/Datasets/LOLdataset/eval/eval15/high/', type=str)
    parser.add_argument('-dirB', default='/mnt/c/Datasets/LOLdataset/eval/eval15/low', type=str)
